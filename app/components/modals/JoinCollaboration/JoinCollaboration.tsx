...
import { ListTodo, Briefcase, Calendar, Users2 } from "lucide-react";
import { useRouter, useSearchParams } from "next/navigation";

import {
  JUNIOR_COLLABORATIONS,
  Collaboration,
  Role,
} from "@data/juniorCollaborations";
...
export const JoinCollaboration = () => {
  ...
  const collab = JUNIOR_COLLABORATIONS.find((c: Collaboration) => c.id === id);

  React.useEffect(() => {
    if (collab && collab.roles.length > 0) {
      setSelectedRole(collab.roles[0].id);
    }
  }, [collab]);
      ...
      <div className="p-6 sm:p-10 pb-4 shrink-0">
        ...
        <div className="border-t border-dashed border-gray-200 mt-6 pt-6 flex flex-wrap items-center gap-x-4 sm:gap-x-6 gap-y-4 text-[10px] sm:text-xs font-medium uppercase tracking-wide">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-900 font-bold">{collab.dueDate}</span>
          </div>
