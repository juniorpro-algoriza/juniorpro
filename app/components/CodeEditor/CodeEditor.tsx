  ...
  return (
    <div className={cx("flex flex-col space-y-1.5", className)}>
      ...
      {isFullscreen ? (
        <div
          className={cx(
            "rounded-2xl border border-dashed border-gray-300 bg-gray-50/60 flex items-center justify-center text-xs text-gray-400 select-none transition-colors",
            isDark ? "border-gray-800 bg-gray-900/30 text-gray-500" : ""
          )}
          style={{ minHeight }}
          aria-hidden="true"
        >
          Editor expanded to fullscreen
        </div>
      ) : (
        editorContainer
      )}

      {/* Fullscreen Overlay via Portal directly to document.body */}
      {isFullscreen &&
        mounted &&
        createPortal(
          <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm p-4 sm:p-6 md:p-8 flex flex-col items-center justify-center animate-fadeIn">
