...
export const InfiniteCarousel = <T,>({
      ...
      const executeItemLoading = async () => {
        try {
          ...
          setItems((currentItems) => {
            ...
            if (newItems.length === 0 || updatedItems.length >= maxItems) {
              setHasMoreToLoad(false);
              listenForScrollRef.current = false;
              if (emblaApi) emblaApi.off("scroll", scrollListenerRef.current);
            } else {
              listenForScrollRef.current = true;
            }
          ...
          // Always reset loading state after request
          setLoadingMore(false);
        }
      };
  ...
  return (
    <section className={cx("bg-white", className)}>
      ...
      {/* Navigation Controls */}
      {(showNavigation || (showViewAll && items.length >= 5)) && (
        <div className="flex justify-between items-center py-4">
          {showNavigation ? renderNavigationButtons() : <div />}
          {showViewAll && items.length >= 5 && renderViewAllButton()}{" "}
        </div>
      )}
    </section>
  );
