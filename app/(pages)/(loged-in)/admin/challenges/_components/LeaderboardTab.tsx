...
import { Button, MainCard, Skeleton } from "@components";
import LeaderboardBg from "@public/images/leaderboard-bg.png";
import WinnersImage from "@public/images/winners.png";
...
type LeaderboardItem =
  components["schemas"]["Sawiha.Services.DTO.AdminChallengeModels.GetLeaderboard.GetAdminChallengeLeaderboardModel"];
...
const LeaderboardCard = ({
  ...
  prizeDistributions?: PrizeDistribution[] | null;
  index: number;
  onView: (participantId: number) => void;
}) => {
  ...
  return (
    ...
    >
      ...
      <div className="pt-7">
        <button
          type="button"
          onClick={() => entry.id && onView(entry.id)}
          disabled={!entry.id}
...
export function LeaderboardTab({
  ...
  const [selectedParticipantId, setSelectedParticipantId] = React.useState<
    number | null
  >(null);
  const [distributionRequested, setDistributionRequested] =
    React.useState(false);
  const { data, isLoading, isFetching, error } = useGetChallengeLeaderboard({
    id: challengeId,
    pageNumber: 1,
    pageSize: 3,
  });
  const distributePrizes = useDistributeChallengePrizes();
  ...
  if (error || leaderboard.length === 0) {
    const handleDistributePrizes = async () => {
      try {
        await distributePrizes.mutateAsync(challengeId);
        setDistributionRequested(true);
        toast.success("Prizes distributed successfully.");
      } catch (error) {
        console.error("Failed to distribute prizes:", error);
        let message = "Failed to distribute prizes. Please try again.";
        try {
    ...
    return (
      <div className="py-6">
        <MainCard classname="flex min-h-[360px] flex-col items-center justify-center overflow-hidden p-10 text-center">
          <Image
            src={WinnersImage}
            alt="Scores finalized"
            width={360}
            height={170}
            className="mb-4 h-auto w-full max-w-[360px] object-contain"
          />

          <h3 className="text-2xl font-extrabold text-[#1F1B72]">
            Scores Finalized!
          </h3>
          <p className="mt-3 max-w-3xl text-sm font-semibold text-slate-500">
            All participants have been successfully evaluated. You are ready to
            publish the winners and distribute the prize pool.
          </p>
          {distributionRequested && isFetching ? (
            <p className="mt-5 text-sm font-semibold text-blue-main">
              Loading winners...
            </p>
          ) : (
            <Button
              intent="main2"
              size="mainDefault"
              className="mt-6 rounded-2xl px-7 shadow-lg shadow-blue-main/20"
              onClick={handleDistributePrizes}
              disabled={distributePrizes.isPending}
            >
        ...
        <div className="relative z-10 grid items-center gap-8 lg:grid-cols-3">
          {orderedTopWinners.map((entry, index) => (
            <LeaderboardCard
              ...
              entry={entry}
              index={index}
              onView={setSelectedParticipantId}
